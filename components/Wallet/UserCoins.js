...
const UserCoins = (prop) => {
    ...
    let queryString = '?start=' + currentRequestOptions.qs.start + '&limit=' + currentRequestOptions.qs.limit + '&convert=' + currentRequestOptions.qs.convert;
    ...
    function MapCoinListWithCurrentValues(coins) {
        ...
        coins.forEach((coin, index) => {
            ...
            if (isCoinMatched) {
                returnList.push({ coin: coin, key: index, currentCoin: currentCoinObj });
            }
        });
        return returnList;
    }
    ...
    const fetchData = async () => {
        try {
            const currentResponse = await axios.get(currentRequestOptions.uri + queryString, {
                headers: currentRequestOptions.headers
            });
            setCurrentCoinList(MapCoinList(Object.values(currentResponse.data.data)));

            const response = await axios.get(requestOptions.uri + '/api/usercoin/getusercoins', {
                headers: requestOptions.headers
            });
            setuserCoins(MapCoinListWithCurrentValues(Object.values(response.data.userCoins)));
            isDataFetched = true;
        } catch (error) {
            console.log(error);
        }
        ...
        if (userCoins.length != 0) {
            return (
                <View style={styles.container}>
                    <AddUserCoinModal userToken={prop.userToken} isReloadNeeded={isReloadNeeded} setIsNeedReload={setIsReloadNeeded} />
